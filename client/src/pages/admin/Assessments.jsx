...
export default function AdminAssessments() {
  ...
  const [editing, setEditing] = useState(null); // assessment being edited
  ...
  const activeCourses = courses.filter(c => c.status !== 'draft');
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['assessments', selectedCourse?.id]); toast.success('Assessment created'); closeModal(); },
    onError: (e) => toast.error(e?.response?.data?.message || 'Failed to create'),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => api.put(`/assessments/${id}`, data),
    onSuccess: () => { qc.invalidateQueries(['assessments', selectedCourse?.id]); toast.success('Assessment saved'); closeModal(); },
    onError: (e) => toast.error(e?.response?.data?.message || 'Failed to save'),
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => api.delete(`/assessments/${id}`),
    onSuccess: () => { qc.invalidateQueries(['assessments', selectedCourse?.id]); toast.success('Deleted'); },
  });
  ...
  return (
    <div className="flex h-full page-enter" style={{ minHeight: 0 }}>
      {/* LEFT — Course list */}
      <div style={{ width: 260, borderRight: '1px solid #e5e7eb', overflowY: 'auto', padding: '24px 0' }}>
        <p style={{ fontSize: 11, fontWeight: 800, letterSpacing: 1.5, color: '#9ca3af', padding: '0 20px 12px' }}>
          LIVE COURSES
        </p>
        {activeCourses.length === 0 && (
          <p style={{ fontSize: 13, color: '#9ca3af', padding: '0 20px' }}>No published courses</p>
        )}
        {activeCourses.map(c => (
          <button
            key={c.id}
        ...
        {!selectedCourse ? (
          ...
          <>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
              ...
              <button
                onClick={openCreate}
                style={{ background: '#16a34a', color: '#fff', border: 'none', borderRadius: 10, padding: '10px 20px', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}
              >
                + Create Assessment
              </button>
            </div>
            ...
            {assessments.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 60, color: '#9ca3af', background: '#f9fafb', borderRadius: 16 }}>
                ...
                <p style={{ marginTop: 12 }}>No assessments yet. Create one to test your students.</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {assessments.map(a => {
                  ...
                  return (
                    <div key={a.id} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <p style={{ fontWeight: 700, fontSize: 15, color: '#0f172a', margin: 0 }}>{a.title}</p>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button onClick={() => togglePublish(a)} style={{ fontSize: 12, padding: '6px 14px', borderRadius: 8, border: '1px solid #d1d5db', background: '#fff', cursor: 'pointer', fontWeight: 600 }}>
                          {a.is_published ? 'Unpublish' : 'Publish'}
                        </button>
                        <button onClick={() => openEdit(a)} style={{ fontSize: 12, padding: '6px 14px', borderRadius: 8, border: '1px solid #d1d5db', background: '#fff', cursor: 'pointer', fontWeight: 600 }}>
                          Edit
                        </button>
                        <button onClick={() => { if (window.confirm('Delete this assessment?')) deleteMutation.mutate(a.id); }} style={{ fontSize: 12, padding: '6px 14px', borderRadius: 8, border: '1px solid #fca5a5', background: '#fff', color: '#dc2626', cursor: 'pointer', fontWeight: 600 }}>
                          Delete
                        </button>
                      </div>
                    </div>
