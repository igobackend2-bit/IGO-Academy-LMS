...
export default function StudentInformation() {
  ...
  const { data: items = [], isLoading } = useQuery({
    queryKey: ['student-resources', 'information'],
    queryFn: () => api.get('/resources/student?type=information').then(r => r.data.data || []),
  });
    ...
    <div style={{ minHeight: '100vh', background: '#f6f8f5' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#0C2014 0%,#16402B 70%,#235C39 100%)', padding: '1.5rem 2rem 2.8rem', color: 'white', position: 'relative', overflow: 'hidden' }}>
        ...
        <div>
          ...
          <h1 style={{ fontSize: '1.5rem', fontWeight: 800, letterSpacing: '-.01em', marginBottom: '.2rem' }}>📋 Information</h1>
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '.8rem' }}>Course materials and important information from IGO Academy</p>
        </div>
      </div>
      ...
      <div style={{ padding: '0 1.75rem 2.5rem', marginTop: '-1.5rem', position: 'relative', zIndex: 1 }}>
        ...
        {isLoading ? (
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: '3.5rem', textAlign: 'center', border: '1px solid #dde5dd', marginTop: '.5rem' }}>
            ...
            <p style={{ color: '#0C2014', fontWeight: 700, fontSize: '1rem', marginBottom: '.35rem' }}>No information posted yet</p>
            <p style={{ color: '#9ca3af', fontSize: '.82rem' }}>Check back soon — your admin will post course materials here.</p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '.5rem' }}>
            {items.map((item, idx) => (
              <div key={item.id} style={{ background: 'white', borderRadius: 16, border: '1px solid #e8f0e8', boxShadow: '0 2px 10px rgba(13,38,25,.05)', overflow: 'hidden' }}>
                ...
                <button onClick={() => toggle(item.id)} style={{ width: '100%', background: 'linear-gradient(135deg,#0C2014,#235C39)', padding: '1rem 1.5rem', display: 'flex', alignItems: 'center', gap: '.75rem', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ color: 'rgba(141,198,63,0.7)', fontSize: '.62rem', fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' }}>Information #{idx + 1}</p>
                  ...
                  <div>
                    ...
                    {item.content && (
                      <div onContextMenu={e => e.preventDefault()}
                        style={{ padding: '1.25rem 1.5rem', userSelect: 'none', WebkitUserSelect: 'none', borderBottom: item.pdf_path ? '1px solid #e8f0e8' : 'none' }}>
                    ...
                    {item.pdf_path && (
                      <div style={{ position: 'relative' }}>
                        <div style={{ background: '#1e3a2a', padding: '.6rem 1.5rem', display: 'flex', alignItems: 'center', gap: '.5rem' }}>
                        ...
                        <div style={{ position: 'relative' }} onContextMenu={e => e.preventDefault()}>
                          <iframe
                            ...
                            title={item.title}
                          />
                          {/* Transparent shield — blocks right-click & drag on the iframe edge */}
